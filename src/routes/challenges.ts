...
router.post('/:challengeId/submit', authenticate, async (req: Request, res: Response) => {
  try {
...
router.get('/:challengeId/hint', authenticate, async (req: Request, res: Response) => {
  try {
    const { challengeId } = req.params;
    const challengeIdStr = Array.isArray(challengeId) ? challengeId[0] : challengeId;
...
router.get('/leaderboard', authenticate, async (req: Request, res: Response) => {
  try {
    const leaderboard = await prisma.userStreak.findMany({
      include: {
        user: {
